...
/** 校验并物化 v2 快照；非法返回 undefined（视为无有效配置） */
...
/**
 * 解码命名空间整段：只读当前版本快照 version-2（Node 半迁移保证启动后段内必有，见 migrateConfig）；
 * 段非法、快照缺失或非法均回退默认。永不返回 undefined（返回 undefined 会让宿主 scope 永挂 loading）。
 */
export function decodeSection(section: unknown): Flags {
    return isPlainObject(section) ? parseV2(section[VERSION_KEY]) ?? DEFAULT_FLAGS : DEFAULT_FLAGS
}
