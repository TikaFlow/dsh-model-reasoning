...
- **自有配置（`model-reasoning` 命名空间）**：通过 `installSettingsSection` 注册到 settings，可在 `settings.yaml` 中编辑或将来通过 Web 设置页开关控制；当前配置项 `allowUpdate`（默认 `false`）为布尔值统一开关，或对象 `{ reasoning, context }` 按字段分别控制是否以 models.dev 最新数据为准更新已有配置（未声明字段等同 `false`，不覆盖已有值）
...
- 填充流程（`update`）：读取 settings 命名空间 `llm-pi-ai` 的 `providers[*].models` 及描述符 revision，对缺少 `reasoningEfforts` / `contextWindow` / `maxTokens` 的模型查找目录（`lookup` 优先按 provider+modelId 匹配，失败再仅按 modelId 全局匹配），生成推理级别或容量值并按 provider 整段写回 `providers[providerId].models` 数组（其余模型字段原样保留；路径 op 不支持数组下标中间段）；覆盖更新仅当 `allowUpdate` 对应字段开启且新值合法（`efforts` 经 `isPlainObject` 校验、容量为数字），新旧值相同则跳过；写回携带 revision 做并发冲突校验，冲突时重读重算（限次）
...
- 自有配置命名空间为 `model-reasoning`（由本插件通过 `installSettingsSection` 注册），在 settings 文件的 `model-reasoning` section 中配置，如 `model-reasoning: { allowUpdate: true }` 或 `model-reasoning: { allowUpdate: { reasoning: true, context: false } }`；后续可为其注册浏览器设置卡片，在 Web 设置页「插件配置」tab 展示开关
