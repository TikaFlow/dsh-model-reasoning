...
- **强制更新（卡片危险按钮 → Node 半，Connection RPC）**：卡片 footer 左侧红底「强制更新」按钮（`--dsw-alias-state-error-primary` 令牌），点击先弹宿主原语 `Modal`+`Button`（来自基线模块 `@deepseek-ai/dsh-client-ui-primitives`，值导入合法、产物走 `require` 模块表；按钮范式同 `RiskConfirmation`，confirm 二次确认，不用浏览器原生弹窗），确认后经 `ctx.get('connection')` 断言后 `rpc.call('/tikaflow-model-fix', 'forceUpdate', {})` 触发 Node 半 `fix(ctx, true)`（语义见「填充流程」段）；`call` 返回 `Promise<RpcResult>`（`{ok,value}|{ok:false,error}`，仅传输层失败才 reject），卡片按变更数/失败原因以宿主原语 `Toast`（同 primitives 模块表）一次性反馈（锚卡片居中、约 4s 自动淡出、失败挂警示图标、seq 自增强刷新重放，message 截断 120 字符）。channel 两端配对：Node 半 `/${PLUGIN_NS}`、浏览器半 `/${MODEL_FIX_NS}` 字面量（禁跨半值导入，改动须同步）；RPC 契约（`RpcResult`/`HostRpcHandle`/`ClientRpcCall`）是宿主 `@deepseek-ai/dsh-client-connection` 的**结构本地复制**，置于 `src/types.ts`——宿主包在 npm 上依赖链不可满足（transitive 范围仅存在于其 monorepo）故不装依赖、仅 type-only 使用（`ctx.get` 断言范式与宿主内置插件 ui-settings-general 一致）；RPC 服务注册与调用走宿主 connection 插件的自定义 channel 能力（≥0.1.0-rc.7 已存在，peer 下限 0.1.2-alpha.2 上实证），信任围栏（loopback/浏览器会话 cookie）由宿主施加，浏览器半 `inject` 含 `connection`
